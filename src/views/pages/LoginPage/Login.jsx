...
export default function Login() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // === MERGE & LƯU USER ĐẦY ĐỦ ===
      // user cũ trong localStorage (nếu trước đó đăng ký đã lưu thêm username/phone/address)
      ...
      const u = data?.user ?? {};
      const mergedUser = {
        id: u.id ?? u._id ?? existingUser?.id ?? existingUser?._id,
        // tên – ưu tiên từ backend
        name: u.name ?? existingUser?.name ?? '',
        email: u.email ?? existingUser?.email ?? email.trim(),
        username: u.username ?? existingUser?.username ?? '',
        phone: u.phone ?? existingUser?.phone ?? '',
        address: u.address ?? existingUser?.address ?? '',
        // createdAt an toàn
        createdAt:
          u.createdAt ??
      ...
      <div className={styles.login}>
        <div className={styles.login__card}>
          ...
          <div className={styles.login__container}>
            ...
            <button type='button' className={`${styles.login__btnSocial} ${styles.facebook}`} disabled>
              <FacebookIcon className={styles.login__icon} />
              <span>Facebook (sắp có)</span>
            </button>
            <button type='button' className={`${styles.login__btnSocial} ${styles.google}`} disabled>
